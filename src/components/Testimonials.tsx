...
const Testimonials = () => {
  ...
  return (
    <section className="py-20 bg-kitchen-DEFAULT text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-4xl font-bold text-center mb-12">לקוחות מספרים</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div 
              key={index} 
              className="bg-white/10 backdrop-blur-sm p-8 rounded-lg border border-white/20 transition-transform duration-300 hover:-translate-y-2"
            >
              <div className="flex mb-3">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <Star key={i} className="fill-kitchen-accent text-kitchen-accent h-5 w-5" />
                ))}
              </div>
